...
@log_step_pbia
@task(name="Load Pipeline Data")
def load_concat_file(config: DictConfig) -> pd.DataFrame:
    path = config.data.rawdailyconcatinated.path
    return (pd.read_csv(path,
                        low_memory=config.data.rawdailyconcatinated.low_memory,
                        header=config.data.rawdailyconcatinated.header_row,
                        sep=config.data.rawdailyconcatinated.delimiter,
                        encoding=config.data.rawdailyconcatinated.encoding,
                        names=config.data.raw_cols)
            )
...
def format_datatype_dates(df: pd.DataFrame, config: DictConfig) -> pd.DataFrame:
    ...
    """
    use_cols = config.data.raw_cols
    date_cols = [col for col in use_cols if 'Time' in col]
    df[date_cols] = df[date_cols].applymap(lambda x: pd.to_datetime(x, format='%Y-%M-%d', errors='coerce',
...
def lengthOfDays_since_lastActive(df) -> pd.DataFrame:
    ...
    df['lengthOfDays_since_lastActive'] = (now - df['max']).astype('timedelta64[D]')
...
def lengthOfDays_since_firstRecord(df) -> pd.DataFrame:
    ...
    df['lengthOfDays_since_firstRecord'] = (now - df['min']).astype('timedelta64[D]')
...
def single_user_frequency(df) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: _description_
    """
    df['frequency'] = round((df['lengthOfDays_since_firstRecord'] - df['lengthOfDays_withActivity']) / df['count'], 2)
    return df
